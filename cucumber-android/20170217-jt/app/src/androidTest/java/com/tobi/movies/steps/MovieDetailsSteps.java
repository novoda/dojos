...
public class MovieDetailsSteps {
    ...
    @When("^I click the movie for id (\\d+)$")
    public void iClickTheMovieForId(int movieId) throws Throwable {
        posterDetailsRobot.launchDetailsScreen(movieId);
    }

    @Then("^I get the detail screen for movie with release date \"([^\"]*)\"$")
    public void iGetTheDetailScreenForMovieWithReleaseDate(String releaseDate) throws Throwable {
        posterDetailsRobot.checkMoviewReleaseDateIsDisplayed(releaseDate);
    }
}
